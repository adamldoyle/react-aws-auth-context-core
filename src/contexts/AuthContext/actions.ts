...
import { IProfile, AuthMode } from './types';
...
export function reducer(state: IAuthState, action: IAuthAction): IAuthState {
  switch (action.type) {
    case 'UPDATE_SESSION':
      ...
      if (
        session.getIdToken().getJwtToken() === state.session?.getIdToken().getJwtToken() &&
        session.getAccessToken().getJwtToken() === state.session.getAccessToken().getJwtToken()
      ) {
        return state;
      }
